import type { NextApiRequest, NextApiResponse } from 'next';
import { OpenAIEmbeddings } from 'langchain/embeddings/openai';
import { PineconeStore } from 'langchain/vectorstores/pinecone';
import { makeChain } from '@/utils/makechain';
import { pinecone } from '@/utils/pinecone-client';
import { PINECONE_INDEX_NAME, PINECONE_NAME_SPACE } from '@/config/pinecone';
import { copyFileSync } from 'fs';
import { getPlayGroundSetting } from './playGroundSetting';
import { json } from 'stream/consumers';

export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse,
) {
  const { question, history,temperature,selectedModel,qaPrompt,condensePrompt,topP,maxLength,presencePenalty,frequencyPenalty,userId } = req.body;

  if (req.method !== 'POST') {
    res.status(405).json({ error: 'Method not allowed' });
    return;
  }

  if (!question) {
    return res.status(400).json({ message: 'No question in the request' });
  }

  // OpenAI recommends replacing newlines with spaces for best results
  var sanitizedQuestion = question.trim().replaceAll('\n', ' ');
  var arabic = /[\u0600-\u06FF]/;
  sanitizedQuestion = arabic.test(sanitizedQuestion)?sanitizedQuestion:sanitizedQuestion + " English";
  try {
    const index = pinecone.Index(PINECONE_INDEX_NAME);

    /* create vectorstore*/
    const vectorStore = await PineconeStore.fromExistingIndex(
      new OpenAIEmbeddings({}),
      {
        pineconeIndex: index,
        textKey: 'text',
        namespace: PINECONE_NAME_SPACE, //namespace comes from your config folder
      },
    );
  
    //create chain
    const chain = makeChain(vectorStore ,userId);//, temperature, selectedModel, qaPrompt, condensePrompt,topP,maxLength,presencePenalty,frequencyPenalty);
    //Ask a question using chat history
    const response = await chain.call({
      question: sanitizedQuestion,
      chat_history: history || [],
    });
    response.text = response.text.trim().replaceAll('\n', ' ');
    console.log('Question: '+ sanitizedQuestion );
    console.log('makeChain: response', response);
    //console.log(response.)
    res.status(200).json(response);
  } catch (error: any) {
    console.log('error', error);
    res.status(500).json({ error: error.message || 'Something went wrong' });
  }
}
